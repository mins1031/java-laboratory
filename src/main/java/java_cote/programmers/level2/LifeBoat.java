...
public class LifeBoat {
    public int solution(int[] people, int limit) {
        ...
        boolean[] mem = new boolean[people.length];

        Arrays.sort(people);
        int backMaxIndex = people.length - 1;
        ...
        for (int i = 0; i < people.length; i++) {
            ...
            for (int j = backMaxIndex; j > 0; j--) {
                int standard = people[0] + people[j];
                if (standard > limit) {
                    continue;
                }
